...
from ..constants import TOTAL_FIELDS, SIG_FIELDS, CUTOFF_TIME
import psycopg2
import os
...
class UfcSpider(scrapy.Spider):
    ...
    # def start_requests(self):
    #     url = "http://www.ufcstats.com/event-details/d26394fc0e8e880a"
    #     yield scrapy.Request(url=url, callback=self.parse_event)

    # Ilia vs Holloway
    # def start_requests(self):
    #     url = "http://www.ufcstats.com/fight-details/ebf7cea27b83c432"
    #     yield scrapy.Request(url=url, callback=self.parse_fight)

    def parse(self, response):
        rows = response.xpath('//tr[@class="b-statistics__table-row"]')[1:]
        ...
        for row in rows:
            ...
            try:
                ...
                # Determine if we go to completed or upcoming
    ...
    def parse_upcoming_event(self, response):
        ...
        eventLoader.add_value('status', response.meta.get("event_status"))
    ...
    def parse_fight(self, response):
        ...
        fightLoader.add_xpath('red_status', "(//i[contains(@class, 'person-status')])[1]/text()")
        fightLoader.add_xpath('blue_status', "(//i[contains(@class, 'person-status')])[2]/text()")
        fightLoader.add_value('winner_id', None)
        fightLoader.add_value('loser_id', None)
        fightLoader.add_value('result_type', None)
        fightLoader.add_value('winner_color', None)       

        # Time and Rounds
        fightLoader.add_xpath('end_round', "//i[contains(text(),'Round')]/following-sibling::text()")
        fightLoader.add_xpath('end_round_time', "normalize-space(//i[contains(text(),'Time')]/following-sibling::text()[1])")
        fightLoader.add_xpath('rounds_scheduled', "normalize-space(//i[i[contains(text(),'Time format')]]/text()[last()])")
        fightLoader.add_xpath('referee', "//i[contains(text(),'Referee')]/following-sibling::span/text()")
        fightLoader.add_value('time_scheduled', None)
        fightLoader.add_value('total_duration', None)

        # Results
        fightLoader.add_xpath('method_raw', "//i[contains(text(),'Method')]/following-sibling::i/text()")
        fightLoader.add_value('finish_type', None)
        fightLoader.add_value('decision_type', None)
    ...
    def parse_fighter_fights(self, response, ffLoaderRed, ffLoaderBlue, fightLoader):
        ...
        for field, td in zip(TOTAL_FIELDS, total_tds):
            if field is None:
                continue
            red_val = td.xpath(f"normalize-space(./p[1]/text())").get()
            blue_val = td.xpath(f"normalize-space(./p[2]/text())").get()
