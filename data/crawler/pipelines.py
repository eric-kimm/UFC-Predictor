...
class ValidationPipeline:
    ...
    def _validate_fight(self, item):
        ...
        required = ['fight_id', 'event_id', 'red_fighter_id', 'blue_fighter_id', 'result_type']
    ...
    def _validate_event(self, item):
        ...
        if item.get('status') is not None:
            if item['status'] not in valid_statuses:
                self.logger.warning(f"EventItem: status '{item['status']}' is not a recognized value")
    ...
    def _validate_fighter_fight(self, item):
        ...
        numeric_fields = ['knockdowns', 'tot_str_landed', 'tot_str_attempted', 
                         ...
                         'distances_str_landed', 'distance_str_attempted', 
                         'clinch_str_landed', 'clinch_str_attempted',
                         'ground_str_landed', 'ground_str_attempted']
        for field in numeric_fields:
            if item.get(field) is not None:
...
class PostgresPipeline:
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            # This will highlight the error in RED or BOLD in many terminals
    ...
    def insert_event(self, data):
        query = """
            INSERT INTO events (event_id, name, date, status, location)
            VALUES (%(event_id)s, %(name)s, %(date)s, %(status)s, %(location)s)
    ...
    def insert_fight(self, data):
        query = """
            INSERT INTO fights (
                ...
                method_raw, finish_type, decision_type, referee
            ) VALUES (
                ...
                %(method_raw)s, %(finish_type)s, %(decision_type)s, %(referee)s
            )
    ...
    def insert_fighter_stats(self, data):
        query = """
            INSERT INTO fighter_fights (
                ...
                ground_str_landed, ground_str_attempted, ground_str_raw
            ) VALUES (
                ...
                %(ground_str_landed)s, %(ground_str_attempted)s, %(ground_str_raw)s
            )
